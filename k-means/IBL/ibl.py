...
class IBL1:
	...
	def train(self,dots):
		...
		for d in dots:
			...
			if nearest_dot.clas == d.clas:
				...
				miss += 1
			self.DC.append(d)
			self.train_dots = dots # saving to use with rand_dots
	...
	def classify(self,dots):
		...
		return classes
	def rand_dots(self, n_rands):
		l = len(self.train_dots[0].pos)
		maxlist=[]
		rands=[]
		for i in range(l):
			lmax = max([d.pos[i] for d in self.train_dots])
			maxlist.append(lmax)
		for dummy in range(n_rands):
			t=[]
			for i in range(l):
				t.append(float(maxlist[i]*random()))
			d=Dot(t)
			rands.append(d)
		return rands
